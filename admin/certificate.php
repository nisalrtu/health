...
try {
    // Get certificate and course information by certificate code
    $stmt = $pdo->prepare("
        SELECT 
            c.certificate_code,
            c.issued_at,
        ...
        GROUP BY c.id, c.certificate_code, c.issued_at, c.verification_url, 
                 co.title, co.description, u.first_name, u.last_name, u.email, u.id, c.course_id
    ");
...
<style>
    ...
    @media print {
        .actions, .sidebar, .navbar, nav, header, .mb-6 { display: none !important; }
    ...
    <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="mb-6 text-center">
            <a href="student-view.php?id=<?php echo $certificate['user_id']; ?>" 
               class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 transition duration-300">
            ...
            <button onclick="window.print()" 
                    class="ml-3 inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white hover:bg-indigo-700 transition duration-300">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
